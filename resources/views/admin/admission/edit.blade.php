                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="basic-form">
                                <form  action="{{route('admission.update', $data->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')

                                    <div class="form-group">
                                        <input type="text" class="form-control input-default " placeholder="Admission Title"
                                            id="admission_title" name="admission_title" value="{{ $data->admission_title }}">
                                        @error('admission_title') <span class="text-danger">{{ $message }}</span> @enderror
                                    </div>
                                    <div class="form-group">
                                        <textarea class="form-control input-default summernote" id="admission_text" name="admission_text"cols="30" rows="10">{!! $data->admission_text !!}</textarea>
